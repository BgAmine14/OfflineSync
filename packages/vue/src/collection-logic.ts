/**
 * Collection state management logic for Vue composables.
 *
 * Plain functions that manage collection state for the useCollection composable.
...
export function createCollectionController<T>(
  ...
  async function refresh(): Promise<void> {
    ...
    try {
      const entities = await dataSource.getAll();
      if (!disposed) {
        internalState.entities = entities;
...
/**
 * Handle a collection entities loaded event.
 *
 * @typeParam T - The shape of the domain-specific data payload.
